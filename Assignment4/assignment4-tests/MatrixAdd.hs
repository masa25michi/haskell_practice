...
sameShape m1 m2 | ((numRows m1) == (numRows m2) && (numColumns m1) == (numColumns m2)) = True
				| otherwise = False

pointwiseApply op m1 m2 | ((sameShape m1 m2) == True) = fromRule ((numRows m1), (numColumns m1)) (\(i,j) -> ((m1 `at` (i,j)) `op` (m2 `at` (i,j))))
						| otherwise = error "They do not have the same shape"
